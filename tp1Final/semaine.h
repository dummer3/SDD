 ...
 *
 * Header contenant toutes les fonctions de base pour la structure semaine
...
typedef struct week {
  char year[5];         // Année de la semaine
 ...
 * \brief Vérifie si l'année est cohérente
 ...
 * \param numWeek : le numéro de semaine à vérifier
 * \return 1 si cohérent, 0 sinon
 *
 **/
int checkNumWeek(char numWeek[2]);
 ...
 * à 52)
 * \return La semaine remplie avec les arguments données s'ils sont corrects,
 * NULL sinon
 *
 **/
 ...
 * un pointeur de pointeur vers l'élément s'il existe, sinon celui qui le suit
 * juste après dans l'ordre de la liste.
 *
 **/
 ...
 * déjà la place)
 *
 * Si la semaine existe déjà, on libère nouvWeek car on n'a pas besoin de
 * l'ajouter.
 *
 **/
 ...
 * \brief supprime la semaine ayant comme information celle fournie en argument
 ...
 * \brief supprime l'action ayant les informations données en argument dans
                          listW s'il existe, sinon ne fait rien.
 * \param listWeek : La liste à regarder
