 ...
 *
 * Fichier contenant la source de toutes les fonctions nécessaires pour la
 * structure semaine
 *
 * Blocs de commentaires expliquant les fonctions sont dans le header pour ne
 *pas surcharger le fichier source
 **/

/**
 *
 * Lexique général :
 * day, hour, year, numWeek : information que va contenir semaine/action.
 * week : La semaine à créer/ supprimer / modifier ...
 * listWeek : La liste de semaine à créer/ supprimer/ modifier ...
 * nouv : Une nouvelle variable allouée
 * prec/ precWeek : pointeur de pointeur de la semaine voulue si elle existe
 **/
...
week_t *newWeek(char year[4], char numWeek[2]) {
  week_t *nouv = NULL; // La nouvelle semaine allouée (null si incorrect)
...
void freeWeek(week_t *week) {
  ...
  free(week);                    // Puis la semaine elle-même
}
...
week_t **findWeek(listWeek_t *listWeek, char year[4], char numWeek[2]) {
  week_t **prec = NULL; // Un pointeur de pointeur de semaine pointant vers la
                        // semaine recherchée si elle existe
...
void supprWeek(listWeek_t *listWeek, char year[4], char week[2]) {
  week_t **prec = findWeek(listWeek, year, week); // Comme dans findWeek

  // Si la semaine correspond bien à celle voulue
  if (prec != NULL && *prec != NULL && equalWeek(*prec, year, week)) {
    // On la supprime
    week_t *tmp = *prec; // pointeur de semaine temporaire
    (*prec) = (*prec)->suiv;
    freeWeek(tmp);
  }
}
...
void insertActionInsideWeek(listWeek_t *listWeek, char year[4], char numWeek[2],
  ...
  // Si nos semaine et action sont cohérentes
